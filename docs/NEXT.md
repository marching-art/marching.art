...
- **Re-score World Championship days 47 and 48 — BEFORE the Day 49 Finals run
  (2026-09-26, 9 PM ET)** — after the functions deploy that carries
  `scoringAwards.championshipConfigForShow`: Admin → Jobs → "Re-score
  Off-Season Day" → 47, confirm; then 48 (in that order — the Day 48 top-25 cut
  is read from the Day 47 recap). Each run re-scores the full field under
  the round's config, rewrites `fantasy_recaps/{seasonUid}/days/{47,48}`,
  refreshes standings and posts the Discord drop + cut. Both nights scored
  nobody, so the re-applied coin awards are the first ones, not doubles.
  Verify: the Scores page shows one ranked World field for each night and the
  run's log line `Day 48 scoring stats` reports ~25 corps scored. Day 49 then
  runs on its own; the only remaining trace is the stored row names, which the
  display layer already covers (`utils/eventNames.ts`). Optional cleanup:
  `patchChampionshipShows` rewrites days 45–49 to the fixed rows with the
  canonical names (it drops the archive date on the World rounds — harmless,
  weather and running orders date from the season calendar).

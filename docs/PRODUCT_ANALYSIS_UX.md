...
| Monetization is ~0% implemented (Stripe webhook is a self-described placeholder; no purchase callables, no premium flags anywhere in code) — so streak-loss and CC-exhaustion dead-ends have no conversion offer behind them. | `functions/src/webhooks/stripe.js:23,68-98`                                                    |

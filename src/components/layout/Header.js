...
const Header = () => {
  ...
  const handleLogout = async () => {
    try {
      await signOut(auth);
      // Optionally clear user profile from Zustand store on logout
  ...
  return (
    <header className="bg-surface-dark text-text-primary-dark shadow-theme-dark sticky top-0 z-50">
        ...
        <div>
          {currentUser ? (
            <div className="relative">
              {/* Basic user display, can be expanded into a dropdown menu */}
             ...
             <button className="bg-primary hover:bg-primary-dark text-on-primary font-bold py-2 px-4 rounded-theme">
                Login
              </button>
          )}
        </div>

  ...
  return (
    <div className="mb-6">
      ...
      {/* Category Tabs - Segmented Control.
          On mobile the buttons share the row evenly (flex-1) with tighter
          padding and a smaller label so all categories fit without horizontal
          scrolling; on sm+ they revert to auto-width with icons and full labels. */}
      <div className="flex items-center gap-1 p-1 bg-surface-sunken border border-line sm:overflow-x-auto">
        {CATEGORIES.map((cat) => {
          const Icon = cat.icon;
          ...
          return (
            <button
              ...
              className={`flex flex-1 sm:flex-initial min-w-0 items-center justify-center gap-1.5 px-1.5 sm:px-3 py-2 text-[9px] sm:text-[10px] font-bold uppercase tracking-wider transition-all whitespace-nowrap ${
                isActive
                  ? 'bg-interactive text-white'
                  : 'text-muted hover:text-secondary hover:bg-white/5'
              }`}
            >
              <Icon className="hidden sm:block w-3 h-3 shrink-0" />

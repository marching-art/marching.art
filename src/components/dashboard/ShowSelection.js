...
const ShowSelection = ({ userProfile }) => {
  ...
  const fetchSeasonSchedule = async () => {
    try {
      ...
      const selectableCompetitions = competitions.filter(comp => {
        // Days 47-49 are auto-enrolled championships
  ...
  const fetchUserRegistrations = async () => {
    try {
      const profileDoc = await getDoc(
        doc(db, `artifacts/${dataNamespace}/users/${currentUser.uid}/profile/data`)
      );
  ...
  const isEligibleForShow = (show) => {
    ...
    if (show.allowedClasses && show.allowedClasses.length > 0) {
      if (!show.allowedClasses.includes(corpsClass)) {
        return {
          eligible: false,
  ...
  const toggleRegistration = async (show) => {
    ...
    try {
      const profileRef = doc(db, `artifacts/${dataNamespace}/users/${currentUser.uid}/profile/data`);
  ...
  const getFilteredShows = (weekShows) => {
    ...
    return weekShows.filter(show => 
      !show.allowedClasses || show.allowedClasses.length === 0 || show.allowedClasses.includes(classFilter)
    );
  };
    ...
    return (
      <div className="flex items-center justify-center min-h-64">
        <div className="text-center">
          ...
          <p className="text-text-secondary dark:text-text-secondary-dark">Loading schedule...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h2 className="text-2xl font-bold text-text-primary dark:text-text-primary-dark mb-2">Competition Registration</h2>
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="text-sm text-text-primary dark:text-text-primary-dark">
            ...
            <ul className="space-y-1 text-text-secondary dark:text-text-secondary-dark">
              ...
              <li>• Your corps class: <span className="font-semibold text-primary dark:text-primary-dark">{corpsClass}</span></li>
            </ul>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-4">
        <div>
          ...
          <select
            ...
            className="bg-surface dark:bg-surface-dark text-text-primary dark:text-text-primary-dark border border-accent dark:border-accent-dark rounded-theme px-4 py-2 focus:outline-none focus:ring-2 focus:ring-primary dark:focus:ring-primary-dark"
          >
      ...
      <div className="space-y-6">
        {filteredWeeks.map(week => {
          ...
          return (
            <div key={week} className="bg-surface dark:bg-surface-dark rounded-theme p-6 border border-accent dark:border-accent-dark">
              {/* Week Header */}
              <div className="flex items-center justify-between mb-4 pb-4 border-b border-accent dark:border-accent-dark">
                <div>
                  <h3 className="text-xl font-bold text-text-primary dark:text-text-primary-dark">Week {week}</h3>
                    ...
                    >
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex-1 min-w-0">
                          <h4 className="font-bold text-text-primary dark:text-text-primary-dark mb-1 truncate">
                            {show.name}
                          </h4>
                      ...
                      {/* Allowed classes */}
                      <div className="text-xs text-text-secondary dark:text-text-secondary-dark mb-3">

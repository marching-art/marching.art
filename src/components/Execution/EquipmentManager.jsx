      ...
      >
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="data-label-sm">Directly affects caption scores (±5% impact)</p>
          </div>
          <div className="flex items-center gap-2 px-3 py-2 bg-black/40 border border-gold-500/30 rounded-lg">
          ...
          {performanceEquipment.map((type, index) => {
            ...
            return (
              <motion.div
                ...
                className="cursor-pointer bg-black/40 border border-white/5 hover:border-gold-500/30 p-3 transition-all group rounded-lg"
                onClick={() => setSelectedEquipment(type.id)}
              >
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 flex items-center justify-center bg-black/40 border border-white/10 text-xl flex-shrink-0 rounded-lg">
                    {type.icon}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1">
                      <h4 className="text-sm font-mono font-bold text-cream-100">{type.name}</h4>
                    ...
                    <p className="data-label-sm mb-1.5">{type.description}</p>
                    <div className="flex items-center gap-2">
                      {/* Segmented Progress Bar */}
                      <div className="flex-1 h-2 bg-charcoal-900 overflow-hidden flex gap-0.5 rounded-sm">
                        {[...Array(10)].map((_, i) => (
                          <div
                            key={i}
                      ...
                      <div className="flex gap-1">
                        {type.affectedCaptions.map(cap => (
                          <span key={cap} className="px-1.5 py-0.5 bg-blue-500/20 border border-blue-500/30 text-[8px] font-mono text-blue-400 rounded">
                            {cap}
                          </span>
                        ))}
                      </div>
                  ...
                  <div className="flex flex-col items-end gap-1">
                    ...
                    <span className="data-label-sm">LVL {level}</span>
                  </div>
                </div>
              </motion.div>
            );
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="data-label-sm">Affects overall travel health (combined 70%+ = no penalty)</p>
          </div>
          {/* Travel Health Status */}
          <div className={`px-3 py-2 border rounded-lg ${hasTravelPenalty ? 'bg-red-500/20 border-red-500/40' : 'bg-green-500/20 border-green-500/40'}`}>
            <div className="flex items-center gap-2">
              {hasTravelPenalty ? (
        ...
        <div className="space-y-2">
          {travelEquipment.map((type, index) => {
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 flex items-center justify-center bg-black/40 border border-white/10 flex-shrink-0 rounded-lg">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="data-label-sm mb-1.5">{type.description}</p>
                    {/* Segmented Progress Bar */}
                    <div className="h-2 bg-charcoal-900 overflow-hidden flex gap-0.5 rounded-sm">
                      {[...Array(10)].map((_, i) => (
                        <div
                          key={i}
                          className={`flex-1 transition-all duration-300 ${

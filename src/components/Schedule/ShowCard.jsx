  ...
  return (
    ...
    >
      ...
      <div className="flex-1 p-3 md:p-4 flex flex-col min-w-0">
        ...
        <div className="mb-2">
          {isLocked ? (
            <span className="inline-flex items-center gap-1 px-2 py-1 text-[10px] font-bold uppercase tracking-wider rounded bg-white/5 text-yellow-50/40 border border-white/10">
              <Lock className="w-3 h-3" />
              {isPast ? 'Closed' : 'Locked'}
            </span>
          ) : show.scoresAvailable ? (
            <span className="inline-flex items-center gap-1 px-2 py-1 text-[10px] font-bold uppercase tracking-wider rounded bg-blue-500/20 text-blue-400 border border-blue-500/30">
              Scores Available
            </span>
          ) : (
            <span className="inline-flex items-center gap-1 px-2 py-1 text-[10px] font-bold uppercase tracking-wider rounded bg-yellow-500/10 text-yellow-400 border border-yellow-500/20">
              Open
            </span>
          )}
        </div>
          ...
          <div className="mb-2">
            <div className="flex flex-wrap gap-1">
              {myCorps.map((corps, idx) => (
                <span
                  key={idx}
                  className="px-2 py-0.5 text-[10px] font-bold rounded bg-yellow-500/15 text-yellow-400 border border-yellow-500/25"
                >
        ...
        <div className="mt-auto pt-2">
          {isPast ? (
            <Link to="/scores" className="block">
              <button className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-lg text-xs font-bold uppercase tracking-wide bg-blue-500/20 text-blue-400 border border-blue-500/30 hover:bg-blue-500/30 transition-all">

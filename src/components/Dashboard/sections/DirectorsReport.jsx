...
/** @typedef {import('../../../utils/directorsReport').DirectorsReportProfile} DirectorsReportProfile */
/** @typedef {import('../../../utils/directorsReport').RecentResult} RecentResult */
/** @typedef {import('../../../utils/directorsReport').PodiumChallengeFacts} PodiumChallengeFacts */
/** @typedef {import('../../../utils/directorsReport').LeaguePoolChallengeFacts} LeaguePoolChallengeFacts */
/** @typedef {import('../../../utils/seasonLadder').LadderProfile} LadderProfile */
...
const DirectorsReportInner = ({
  ...
  // The store's ProfileDoc is the wider shape; the resolver and the ladder
  // each read a documented slice of it.
  const profile = /** @type {(DirectorsReportProfile & LadderProfile) | null} */ (
    useProfileStore((state) => state.profile)
  );
  const [claimingTier, setClaimingTier] = useState(/** @type {number | null} */ (null));
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <ClipboardList className={`w-3.5 h-3.5 ${allDone ? 'text-interactive' : 'text-white'}`} />
          Director&apos;s Report
        </h3>
        <span
...
const DirectorsReport = memo(DirectorsReportInner);

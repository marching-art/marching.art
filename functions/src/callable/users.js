 ...
 * Handles user profile updates, XP awards, and class unlocks
 * 
 * Location: functions/src/callable/users.js
 */
 ...
 */
exports.updateCorpsInfo = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    try {
      ...
      // Add optional fields
      if (location) {
        updates["corps.location"] = location.trim();
      }

      if (corpsClass) {
        updates["corps.corpsClass"] = corpsClass;
      }
...
/**
 * Updates uniform settings for a user
 */
exports.updateUniform = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    if (!uniformData) {
      throw new functions.https.HttpsError("invalid-argument", "Uniform data is required.");
    }
 ...
 */
exports.checkClassUnlocks = functions
  .runWith(getFunctionConfig('light'))
  .https.onCall(async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError("unauthenticated", "You must be logged in.");

...
async function processPodiumDay(db, seasonData, { calendarDay, competitionDay }) {
  ...
  try {
    ...
    // fantasy `displayName` shape) and attach the corps avatar (stored per class
    // at `corps.podiumClass.avatarUrl`, same source the fantasy classes use) so
    // the Scores sheet can name, link, and picture them. One batched getAll over
    // the show-day corps — never a read per row, and never fails the night.
    ...
    if (recapUids.length > 0) {
      try {
        ...
        profileSnaps.forEach((snap, i) => {
          ...
          avatarByUid[recapUids[i]] = (data && data.corps?.podiumClass?.avatarUrl) || null;
        });
        for (const show of recapShows) {
          for (const entry of show.results) {
            entry.displayName = nameByUid[entry.uid] || null;
            entry.avatarUrl = avatarByUid[entry.uid] || null;
          }
        }
      } catch (error) {

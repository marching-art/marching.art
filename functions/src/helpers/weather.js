...
/**
 * A Date (or parseable value) as a local YYYY-MM-DD string, or null. A value
 * that already IS a `YYYY-MM-DD` calendar day passes through untouched: the
 * schedule producer dates shows from the season calendar in that form
 * (gameDay.competitionDayToIsoDate), and round-tripping it through `new Date`
 * would read the UTC-midnight instant with local getters and land a day early
